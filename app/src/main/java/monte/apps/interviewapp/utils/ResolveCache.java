...
/**
 * Internally hold a cache of scaled icons based on {@link PackageManager} queries, keyed internally
 * on MIME-type.
 */
public class ResolveCache {
    /**
     * Specific list {@link ApplicationInfo#packageName} of apps that are prefered
     * <strong>only</strong> for the purposes of default icons when multiple {@link ResolveInfo} are
     * found to match. This only happens when the user has not selected a default app yet, and they
     * will still be presented with the system disambiguation dialog. If several of this list match
     * (e.g. Android Browser vs. Chrome), we will pick either one
     */
    private static HashSet<String> sPreferResolve;
    ...
    /**
     * Returns an instance of the ResolveCache. Only one internal instance is kept, so the argument
     * packageManagers is ignored for all but the first call
     */
    public synchronized static ResolveCache getInstance(Context context) {
        if (sInstance == null) {
            ...
            sPreferResolve = new HashSet<>();
            sPreferResolve.add("com.android.email");
            sPreferResolve.add("com.google.android.email");
            sPreferResolve.add("com.android.phone");
            sPreferResolve.add("com.google.android.apps.maps");
            sPreferResolve.add("com.android.chrome");
            sPreferResolve.add("com.google.android.browser");
            sPreferResolve.add("com.android.browser");
    ...
    /**
     * Get the {@link Entry} best associated with the given mimetype and intent, or create and
     * populate a new one if it doesn't exist.
     */
    ...
    /**
     * Best {@link ResolveInfo} when multiple found. Ties are broken by selecting first from the
     * {@link ResolveCache#sPreferResolve} list of preferred packages, second by apps that live on
     * the system partition, otherwise the app from the top of the list. This is
     * <strong>only</strong> used for selecting a default icon for displaying in the track, and does
     * not shortcut the system {@link Intent} disambiguation dialog.
     */
    ...
    /**
     * Check {@link PackageManager} to see if any apps offer to handle the given {@link Intent}.
     */
    ...
    /**
     * Return the best icon for the given {@code mimeType} and {@code intent}, which is usually
     * based on the {@link ResolveInfo} found through a {@link PackageManager} query.
     */
    ...
    /**
     * Cached entry holding the best {@link ResolveInfo} for a specific MIME-type, along with a
     * {@link SoftReference} to its icon.
     */
